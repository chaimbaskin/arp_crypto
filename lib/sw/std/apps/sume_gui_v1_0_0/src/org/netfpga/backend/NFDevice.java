...
/*	IntByReference val = new IntByReference();
    ...
    public int readReg(long addr) {
    	/* create pointer to int */
	IntByReference val = new IntByReference();
	IntByReference fed = new IntByReference();
	NFRegAccess.INSTANCE.regread(fed, (int)addr, val);
	int retVal = val.getValue();
    	return retVal;
    }


    public int regread(long addr) {
	int retVal = NFRegAccess.INSTANCE.regread(addr);
   	return retVal;
    }
    ...
    public void writeReg(long addr, int val) {
    	NFRegAccess.INSTANCE.regwrite(this.nf2.fd, (int)addr, val);
    }
  ...
  /* public int checkIface() {
    	return 1;//NFRegAccess.INSTANCE.check_iface(nf2);
    }
*/
    /**
